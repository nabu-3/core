...
/**
 * Class to manage a list of Site Target CTA Role instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\site\base
 */
abstract class CNabuSiteTargetCTARoleListBase extends CNabuDataObjectList
{
    /**
    ...
    /**
     * Acquires an instance of class CNabuSiteTargetCTARole from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuSiteTargetCTARole($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
