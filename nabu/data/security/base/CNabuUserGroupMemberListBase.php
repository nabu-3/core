...
/**
 * Class to manage a list of User Group Member instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\security\base
 */
abstract class CNabuUserGroupMemberListBase extends CNabuDataObjectList
{
    /**
    ...
    {
        parent::__construct('nb_user_group_id');
    }
    ...
    /**
     * Acquires an instance of class CNabuUserGroupMember from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuUserGroupMember($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
