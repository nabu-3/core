 ...
 * ---------------------------------------------------------------------------
 * Created: 2018/01/12 12:35:05 UTC
 * ===========================================================================
...
/**
 * Class to manage a list of Catalog Taxonomy instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\catalog\base
 */
abstract class CNabuCatalogTaxonomyListBase extends CNabuDataObjectList
{
    /**
     * Index the list using the nb_catalog_taxonomy_key field.
     * @var string
     */
    ...
    {
        parent::__construct('nb_catalog_taxonomy_id');
    }
    ...
    {
        $this->addIndex(
            new CNabuDataObjectListIndex($this, 'nb_catalog_taxonomy_key', 'nb_catalog_taxonomy_key', self::INDEX_KEY)
        );
    }

    /**
     * Acquires an instance of class CNabuCatalogTaxonomy from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuCatalogTaxonomy($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
