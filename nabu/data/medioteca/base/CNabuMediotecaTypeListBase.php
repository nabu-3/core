 ...
 * ---------------------------------------------------------------------------
 * Created: 2018/08/22 05:46:52 UTC
 * ===========================================================================
...
/**
 * Class to manage a list of Medioteca Type instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\medioteca\base
 */
abstract class CNabuMediotecaTypeListBase extends CNabuDataObjectList
{
    /**
     * Index the list using the nb_medioteca_type_key field.
     * @var string
     */
    ...
    {
        $this->addIndex(
            new CNabuDataObjectListIndex($this, 'nb_medioteca_type_key', 'nb_medioteca_type_key', self::INDEX_KEY)
        );
    }

    /**
     * Acquires an instance of class CNabuMediotecaType from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuMediotecaType($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
