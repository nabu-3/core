 ...
 * ---------------------------------------------------------------------------
 * Created: 2017/12/04 11:58:28 UTC
 * ===========================================================================
...
/**
 * Class to manage a list of Medioteca Item instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\medioteca\base
 */
abstract class CNabuMediotecaItemListBase extends CNabuDataObjectList
{
    /**
     * Index the list using the nb_medioteca_item_key field.
     * @var string
     */
    const INDEX_KEY = "keys";

    /**
     * Index the list using the nb_medioteca_item_order field.
     * @var int
     */
    ...
    {
        parent::__construct('nb_medioteca_item_id');
    }
    ...
    {
        $this->addIndex(
            new CNabuDataObjectListIndex($this, 'nb_medioteca_item_key', 'nb_medioteca_item_order', self::INDEX_KEY)
        );
        $this->addIndex(
            new CNabuDataObjectListIndex($this, 'nb_medioteca_item_order', 'nb_medioteca_item_order', self::INDEX_ORDER)
        );
    }

    /**
     * Acquires an instance of class CNabuMediotecaItem from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuMediotecaItem($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
