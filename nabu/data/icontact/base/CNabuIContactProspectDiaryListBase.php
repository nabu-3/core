...
/**
 * Class to manage a list of iContact Prospect Diary instances.
 * @author Rafael Gutiérrez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\icontact\base
 */
abstract class CNabuIContactProspectDiaryListBase extends CNabuDataObjectList
{
    /**
    ...
    {
        parent::__construct('nb_icontact_prospect_diary_id');
    }
    ...
    /**
     * Acquires an instance of class CNabuIContactProspectDiary from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuIContactProspectDiary($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
