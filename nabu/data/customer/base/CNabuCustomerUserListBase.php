...
/**
 * Class to manage a list of Customer User instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\customer\base
 */
abstract class CNabuCustomerUserListBase extends CNabuDataObjectList
{
    /**
    ...
    /**
     * Acquires an instance of class CNabuCustomerUser from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuCustomerUser($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
