...
/**
 * Class to manage a list of Commerce Product instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\commerce\base
 */
abstract class CNabuCommerceProductListBase extends CNabuDataObjectList
{
    /**
    ...
    {
        parent::__construct('nb_commerce_product_id');
    }
    ...
    /**
     * Acquires an instance of class CNabuCommerceProduct from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuCommerceProduct($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
