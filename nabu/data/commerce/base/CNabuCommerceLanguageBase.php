 ...
 * ---------------------------------------------------------------------------
 * Created: 2017/03/28 17:12:30 UTC
 * ===========================================================================
...
use \nabu\data\commerce\CNabuCommerceLanguageList;
...
/**
 * Class to manage the entity Commerce Language stored in the storage named nb_commerce_lang.
 * @author Rafael Gutiérrez Martínez <rgutierrez@wiscot.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\commerce\base
 */
abstract class CNabuCommerceLanguageBase extends CNabuDBInternalObject implements INabuTranslation
{
     ...
     * @param mixed $nb_commerce An instance of CNabuCommerceLanguageBase or another object descending from
     * \nabu\data\CNabuDataObject which contains a field named nb_commerce_id, or a valid ID.
     * @param mixed $nb_language An instance of CNabuCommerceLanguageBase or another object descending from
     * \nabu\data\CNabuDataObject which contains a field named nb_language_id, or a valid ID.
     */
    public function __construct($nb_commerce = false, $nb_language = false)
    {
        if ($nb_commerce) {
    ...
    {
        return ($this->isValueNumeric('nb_commerce_id') && $this->isValueNumeric('nb_language_id'))
            ? $this->buildSentence(
                    'select * '
                    . 'from nb_commerce_lang '
    ...
    {
        $nb_commerce_id = nb_getMixedValue($translated, 'nb_commerce_id');
        if (is_numeric($nb_commerce_id)) {
            $retval = CNabuLanguage::buildObjectListFromSQL(
                    'nb_language_id',
                    'select l.* '
                    . 'from nb_language l, nb_commerce t1, nb_commerce_lang t2 '
                   . 'where t1.nb_commerce_id=t2.nb_commerce_id '
                     . 'and t1.nb_commerce_id=%nb_commerce_id$d '
                     . 'and l.nb_language_id=t2.nb_language_id ',
                    array(
                        'nb_commerce_id' => $nb_commerce_id
                    )
            );
        } else {
     ...
     * @return CNabuCommerceLanguageList Returns a list of translations. If no translations are available, the list is
     * empty.
     */
    public static function getTranslationsForTranslatedObject($translated)
    {
        $nb_commerce_id = nb_getMixedValue($translated, 'nb_commerce_id');
        if (is_numeric($nb_commerce_id)) {
            $retval = CNabuCommerceLanguage::buildObjectListFromSQL(
                    'nb_language_id',
                    'select t2.* '
                    . 'from nb_language l, nb_commerce t1, nb_commerce_lang t2 '
                   . 'where t1.nb_commerce_id=t2.nb_commerce_id '
            ...
            $retval = new CNabuCommerceLanguageList();
        }
     ...
     * @return CNabuCommerceLanguageBase Returns $this
     */
    ...
    /**
     * Get Commerce Lang Name attribute value
     * @return null|string Returns the Commerce Lang Name value
     */
    public function getName()
    {
        return $this->getValue('nb_commerce_lang_name');
    }

    /**
     * Sets the Commerce Lang Name attribute value
     * @param null|string $name New value for attribute
     * @return CNabuCommerceLanguageBase Returns $this
     */
    public function setName($name)
    {
        $this->setValue('nb_commerce_lang_name', $name);
