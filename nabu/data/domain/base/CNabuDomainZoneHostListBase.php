...
/**
 * Class to manage a list of Domain Zone Host instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\domain\base
 */
abstract class CNabuDomainZoneHostListBase extends CNabuDataObjectList
{
    /**
    ...
    {
        parent::__construct('nb_domain_zone_host_id');
    }
    ...
    /**
     * Acquires an instance of class CNabuDomainZoneHost from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuDomainZoneHost($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
