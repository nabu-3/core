 ...
 * ---------------------------------------------------------------------------
 * Created: 2018/02/22 05:37:59 UTC
 * ===========================================================================
...
/**
 * Class to manage a list of Messaging Template instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\messaging\base
 */
abstract class CNabuMessagingTemplateListBase extends CNabuDataObjectList
{
    /**
     * Index the list using the nb_messaging_template_key field.
     * @var string
     */
    ...
    {
        parent::__construct('nb_messaging_template_id');
    }
    ...
    {
        $this->addIndex(
            new CNabuDataObjectListIndex($this, 'nb_messaging_template_key', 'nb_messaging_template_key', self::INDEX_KEY)
        );
    }

    /**
     * Acquires an instance of class CNabuMessagingTemplate from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuMessagingTemplate($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
