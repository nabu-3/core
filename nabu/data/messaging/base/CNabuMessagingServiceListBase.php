...
/**
 * Class to manage a list of Messaging Service instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@nabu-3.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\messaging\base
 */
abstract class CNabuMessagingServiceListBase extends CNabuDataObjectList
{
    /**
     * Index the list using the nb_messaging_service_key field.
     * @var string
     */
    ...
    {
        parent::__construct('nb_messaging_service_id');
    }
    ...
    {
        $this->addIndex(
            new CNabuDataObjectListIndex($this, 'nb_messaging_service_key', 'nb_messaging_service_key', self::INDEX_KEY)
        );
    }

    /**
     * Acquires an instance of class CNabuMessagingService from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuMessagingService($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
