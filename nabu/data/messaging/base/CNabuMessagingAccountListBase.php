 ...
 * ---------------------------------------------------------------------------
 * Created: 2017/02/27 16:28:51 UTC
 * ===========================================================================
...
/**
 * Class to manage a list of Messaging Account instances.
 * @author Rafael Gutiérrez Martínez <rgutierrez@wiscot.com>
 * @version 3.0.12 Surface
 * @package \nabu\data\messaging\base
 */
abstract class CNabuMessagingAccountListBase extends CNabuDataObjectList
{
    /**
    ...
    {
        parent::__construct('nb_messaging_account_id');
    }
    ...
    {
        $this->addIndex(
            new CNabuDataObjectListIndex($this, 'nb_messaging_account_key', 'nb_messaging_account_order', self::INDEX_KEY)
        );
    }

    /**
     * Acquires an instance of class CNabuMessagingAccount from the database.
    ...
    public function acquireItem($key, $index = false)
    {
        ...
        if ($index === false && CNabuEngine::getEngine()->isMainDBAvailable()) {
            $item = new CNabuMessagingAccount($key);
            if ($item->isFetched()) {
                $retval = $item;
            }
        }
