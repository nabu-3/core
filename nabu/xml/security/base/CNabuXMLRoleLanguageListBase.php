 ...
 * ---------------------------------------------------------------------------
 * Created: 2019/04/04 14:15:48 UTC
 * ===========================================================================
...
abstract class CNabuXMLRoleLanguageListBase extends CNabuXMLTranslationsList
{
    ...
    protected function locateDataObject(SimpleXMLElement $element, CNabuDataObject $data_parent = null) : bool
    {
        ...
        if (isset($element['GUID'])) {
            ...
            if (!($this->nb_data_object instanceof CNabuRoleLanguageList)) {
                $this->nb_data_object = CNabuRoleLanguageList::findByHash($guid);
            } else {
                $this->nb_data_object = null;
            }
        
            if (!($this->nb_data_object instanceof CNabuRoleLanguageList)) {
                $this->nb_data_object = new CNabuRoleLanguageList();
