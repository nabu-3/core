...
abstract class CNabuXMLSiteMapRoleListBase extends CNabuXMLDataObjectList
{
    ...
    protected function locateDataObject(SimpleXMLElement $element, CNabuDataObject $data_parent = null) : bool
    {
        ...
        if (isset($element['GUID'])) {
            ...
            if (!($this->nb_data_object instanceof CNabuSiteMapRoleList)) {
                $this->nb_data_object = CNabuSiteMapRoleList::findByHash($guid);
            } else {
                $this->nb_data_object = null;
            }
        
            if (!($this->nb_data_object instanceof CNabuSiteMapRoleList)) {
                $this->nb_data_object = new CNabuSiteMapRoleList();
