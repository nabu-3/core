 ...
 * ---------------------------------------------------------------------------
 * Created: 2018/08/20 09:23:01 UTC
 * ===========================================================================
...
use \nabu\data\site\CNabuSiteTargetCTAList;
use \nabu\xml\CNabuXMLDataObject;
use \nabu\xml\CNabuXMLDataObjectList;
use \nabu\xml\site\CNabuXMLSiteTargetCTA;
 ...
 * @package \nabu\xml\site\base
 */
abstract class CNabuXMLSiteTargetCTAListBase extends CNabuXMLDataObjectList
{
    /**
     * Instantiates the class. Receives as parameter a qualified CNabuSiteTargetCTAList class.
     * @param CNabuSiteTargetCTAList $nb_site_target_cta $this->entity_name instance to be managed as XML
     */
    public function __construct(CNabuSiteTargetCTAList $nb_site_target_cta)
    {
    ...
    {
        return new CNabuXMLSiteTargetCTA($nb_child);
    }
    ...
    protected function locateDataObject(SimpleXMLElement $element, CNabuDataObject $data_parent = null) : bool
    {
        ...
        if (isset($element['GUID'])) {
            ...
            if (!($this->nb_data_object instanceof CNabuSiteTargetCTAList)) {
                $this->nb_data_object = CNabuSiteTargetCTAList::findByHash($guid);
            } else {
                $this->nb_data_object = null;
            }
        
            if (!($this->nb_data_object instanceof CNabuSiteTargetCTAList)) {
                $this->nb_data_object = new CNabuSiteTargetCTAList();
