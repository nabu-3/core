 ...
 * @package \nabu\xml\site\base
 */
abstract class CNabuXMLSiteListBase extends CNabuXMLDataObjectList
{
    /**
     * Instantiates the class. Receives as parameter a qualified CNabuSiteList class.
     * @param CNabuSiteList $nb_site $this->entity_name instance to be managed as XML
     */
    public function __construct(CNabuSiteList $nb_site)
    {
    ...
    protected function locateDataObject(SimpleXMLElement $element, CNabuDataObject $data_parent = null) : bool
    {
        ...
        if (isset($element['GUID'])) {
            ...
            if (!($this->nb_data_object instanceof CNabuSiteList)) {
                $this->nb_data_object = CNabuSiteList::findByHash($guid);
            } else {
                $this->nb_data_object = null;
            }
        
            if (!($this->nb_data_object instanceof CNabuSiteList)) {
                $this->nb_data_object = new CNabuSiteList();
