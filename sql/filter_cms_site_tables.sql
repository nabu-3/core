...
select * from nb_site_role where nb_site_id=1 order by nb_role_id
;

select * from nb_site_role_lang where (nb_site_id, nb_role_id) in (select nb_site_id, nb_role_id from nb_site_role where nb_site_id=1) order by nb_language_id
;
